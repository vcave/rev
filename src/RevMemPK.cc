...
extern size_t page_freelist_depth; // Idx 
...
RevMem::RevMem( unsigned long MemSize, RevOpts *Opts, SST::Output *Output )
  : opts(Opts), ctrl(nullptr),output(Output),
    stacktop(0x00ull) {
...
// TODO: Handle Fault Injection
// - Maybe extern root_page_table
// - pick rand page of memory 
void RevMem::HandleMemFault(unsigned width){
  ...
  std::uniform_int_distribution<> distr(0, page_freelist_depth); // define the range
  unsigned PageIdx = distr(gen);
  // uint64_t *Addr = (uint64_t *)(&physMem[0] + NBytes);
  uint64_t Addr = PageIdx * RISCV_PGSIZE;
  // memset_user(static_cast<void*> Addr, int ch, size_t n)
  uint64_t Value = 0;
  ReadMem(Addr, sizeof(uint64_t), &Value);
  
  // Value is 0 until response from MemHierarchy
  Value |= rval;

  WriteMem(Addr, sizeof(uint64_t), &Value);
...
bool RevMem::WriteMem( uint64_t Addr, size_t Len, void *Data ){
  memcpy_from_user(reinterpret_cast<void*>(Addr), Data, Len);
  return true;
}
// #ifdef _REV_DEBUG_
...
bool RevMem::ReadMem( uint64_t Addr, size_t Len, void *Data ){
  memcpy_to_user(reinterpret_cast<void*>(Addr), Data, Len);
  return true;
}
// #ifdef _REV_DEBUG_
